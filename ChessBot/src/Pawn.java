...
public class Pawn implements Piece {
     ...
     * @param color the color of the piece Constructor for the Pawn class,
    ...
    public ArrayList<Board> findMoves(Board b, int index) {
        ...
        if (pieces.get(index).getColor() == true) {
            ...
            // if (curY == 4) {
            //     if (prevBoard.check(new Location(curX + 1, curY)) && b.getPiece(new Location(curX + 1, curY)) != null
            //             && b.getPiece(new Location(curX + 1, curY)) instanceof Pawn) {
            //         Board copy = new Board(b.copyBoard());
            //         copy.updateBoard(copy.getPiece(new Location(curX + 1, curY)), null);
            //         copy.updateBoard(copy.getPiece(new Location(this.getLoc().getXPos(), this.getLoc().getYPos())),
            //                 new Pawn(this.getLoc().getXPos() + 1, this.getLoc().getYPos() + 1, color));
            //         ans.add(copy);
            //         System.out.println("en passant boiii");
        ...
        if (pieces.get(index).getColor() == true) {
            for (Piece i : pieces) {
                if (i instanceof King && i.getColor() == false) {
                    if ((i.getLoc().getXPos() == curX + 1 && i.getLoc().getYPos() == curY + 1)
                            || (i.getLoc().getXPos() == curX - 1 && i.getLoc().getYPos() == curY + 1)) {
                        return true;
                    } else {
                        return false;
                    }
